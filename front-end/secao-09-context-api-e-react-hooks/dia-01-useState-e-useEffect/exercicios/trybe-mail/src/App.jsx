...
function App() {
  ...
  return (
    <section className="App">
      ...
      <button onClick={() => handleReadOrUnreadEmail(READ_ALL)}>
        Marcar todas como lida
      </button>

      <button onClick={() => handleReadOrUnreadEmail(UNREAD_ALL)}>
      ...
      <ul>
        {emails.map(({ id, title, status }) => (
          <li key={id}>
            <p>{`${title} ${status ? "(Lido)" : "(Não lido)"}`}</p>

            {status ? (
              <button onClick={() => handleReadOrUnreadEmail(UNREAD_EMAIL, id)}>
                Marcar como não lido
              </button>
            ) : (
              <button onClick={() => handleReadOrUnreadEmail(READ_EMAIL, id)}>
                Marcar como lido
              </button>
            )}
          </li>
